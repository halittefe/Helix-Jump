...
public class HelixManager1 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            if (i == 0) // spawn first ring
            {
                SpawnRings(0);
            }
        else // spawn the middle rings except the 0, the last one
            {
                SpawnRings(Random.Range(1, rings.Length - 1));
            }
       
        }
        SpawnRings(rings.Length - 1); // last ring spawn
    }
